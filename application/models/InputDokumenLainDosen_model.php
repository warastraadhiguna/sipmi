...
class InputDokumenLainDosen_model extends MY_Model
{
    protected $table = 'tdetaildokumenlaindosen';
    protected $order = 'DESC';
    private $idUser;
    private $dataTahunPelaksanaan;
    ...
    public function json()
    {
        ...
        $this->datatables->where("tdetaildokumenlaindosen.idTahunPelaksanaan", $this->dataTahunPelaksanaan->ID);
        $this->datatables->where("tdetaildokumenlaindosen.idJenisDokumenLain", $idJenisDokumenLain);
        $this->datatables->where("tdetaildokumenlaindosen.idUserDosen", $this->session->userdata['ID']);
        $this->datatables->add_column(
            'action',
            anchor(
                site_url('InputDokumenLainDosen/update/$1'),
                '<button type="button" class="btn btn-warning" title="Edit Data"><i class="fa fa-pencil" aria-hidden="true"></i></button>'
            )." ".
            anchor(
                site_url('InputDokumenLainDosen/delete/$1'),
